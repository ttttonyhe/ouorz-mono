...
const SubscriptionBox = ({
	...
	if (type === "sm") {
		return (
			<div
				...
				className="my-2 flex w-full flex-col gap-y-3 rounded-md border bg-white px-4.5 py-4 shadow-xs dark:border-gray-700 dark:bg-gray-800 lg:flex-row lg:items-center lg:gap-x-10 lg:gap-y-0 lg:space-x-4 lg:py-3">
				<div>
					...
					{subscribed ? (
						...
						<form
							...
							<input
								...
								} flex h-8 w-full rounded-md border bg-white px-4 text-4 tracking-wide text-gray-500 shadow-sm focus:outline-hidden dark:border-gray-700 dark:bg-gray-700 dark:text-gray-400`}
							...
							{email && email.length <= 23 && (
								<button
									type="submit"
									className="effect-pressing absolute right-1.5 top-[6.5px] hidden animate-appear rounded-full bg-slate-500 px-[8px] py-[2px] text-xs text-white shadow-xs hover:bg-neutral-600 hover:shadow-inner dark:bg-neutral-800 dark:text-gray-300 lg:block">
									Subscribe
								</button>
							)}
							<button
								type="submit"
								className="effect-pressing absolute right-1.5 top-[6.5px] animate-appear rounded-full bg-slate-500 px-[8px] py-[2px] text-xs text-white shadow-xs hover:bg-neutral-600 hover:shadow-inner dark:bg-neutral-800 dark:text-gray-300 lg:hidden">
								Subscribe
							</button>
						</form>
					)}
	...
	return (
		<div
			className={`my-2 hidden w-full items-center rounded-xl border bg-white p-10 shadow-xs dark:border-gray-800 dark:bg-gray-800 lg:block lg:px-20 lg:py-11 ${className || ""}`}>
			...
			<div className="grid w-full grid-cols-3 gap-5">
				<form
					...
					<input
						type="email"
						value={email}
						className="col-start-1 col-end-3 w-full rounded-bl-md rounded-tl-md border border-r-0 border-gray-200 px-4 py-2 font-light shadow-xs focus:border-gray-300 focus:outline-hidden dark:border-gray-500 dark:bg-gray-600"
					...
					/>
					{subscribed ? (
						<div className="col-start-3 col-end-4 flex cursor-pointer items-center rounded-br-md rounded-tr-md border border-green-600 bg-green-500 text-center text-green-50 shadow-xs">
							<span className="mx-auto">Success!</span>
						</div>
					) : (
						<button
							type="submit"
							className="col-start-3 col-end-4 flex cursor-pointer items-center rounded-br-md rounded-tr-md border border-blue-200 bg-blue-50 text-center text-blue-500 shadow-xs hover:border-blue-300 hover:bg-blue-100 dark:border-blue-400 dark:bg-blue-500 dark:text-white dark:hover:border-blue-400 dark:hover:bg-blue-600"
							onClick={() => {
								email && doSubscribe()
							}}>
