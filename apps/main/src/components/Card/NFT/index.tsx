...
const NFTCard = (props: PropsType) => {
	...
	return (
		<div className="glowing-div w-50 z-40 flex cursor-pointer items-center rounded-md border bg-white shadow-sm transition-shadow hover:shadow-md dark:border-none dark:bg-gray-700">
			<GlowingBackground />
			<div className="glowing-div-content bottom-0 left-0 right-0 top-[0.5px] h-full overflow-hidden rounded-[5px]">
				<div
					...
					<div className="absolute top-3 z-20 flex w-full justify-between px-3 text-gray-600 dark:text-gray-100">
						<label className="rounded-full bg-white p-[1.5px] shadow-sm dark:bg-gray-700 dark:bg-opacity-50">
							<span className="flex h-[1rem] w-[1rem]">
								<Icon name={icon} />
							</span>
						</label>
						{props.tokenType && (
							<label className="z-20 flex items-center rounded-full bg-white px-2 py-0.5 text-xs shadow-sm dark:bg-gray-700 dark:bg-opacity-50">
								{props.tokenType}
							</label>
						)}
					</div>
				...
				<div className="px-3.5 pb-5">
					<div className="z-30 -mt-3.5 mb-3 flex w-full justify-center">
						<a
							className="inline-block w-28 overflow-hidden text-ellipsis rounded-full border border-gray-300 bg-gray-100 px-2 py-0.5 text-sm text-gray-600 shadow-sm dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300"
							href={tokenLink}
							target="_blank"
							rel="noreferrer">
...
const NFTCardLoading = (props: { uniqueKey: string }) => {
	return (
		<div className="glowing-div w-50 z-40 flex items-center rounded-md border bg-white p-[1px] shadow-sm transition-shadow hover:shadow-md dark:border dark:border-gray-700 dark:bg-gray-800">
			<ContentLoader
