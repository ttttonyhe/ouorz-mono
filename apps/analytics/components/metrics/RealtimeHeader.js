...
export default function RealtimeHeader({
	...
	const count = useMemo(() => {
		return sessions.filter(
			({ created_at }) =>
				differenceInMinutes(new Date(), new Date(created_at)) <= 5
		).length
	}, [sessions, websiteId])
	...
	return (
		<>
			...
			<div className={styles.metrics}>
				...
				<MetricCard
					label={
						<FormattedMessage id="metrics.visitors" defaultMessage="Visitors" />
					}
				...
				<MetricCard
					label={
						<FormattedMessage id="metrics.events" defaultMessage="Events" />
					}
					value={events.length}
					hideComparison
				/>
